...
class ConnectionDetailInfoView: NSView {
    ...
    let processNameLabel = NSTextField(labelWithString: "")
    let hostLabel = NSTextField(labelWithString: "")
    let generalView = ConnectionDetailInfoGeneralView.createFromNib()
    ...
    func setupSubviews() {
        ...
        separator.makeConstraints {
            [$0.heightAnchor.constraint(equalToConstant: 1),
             $0.leftAnchor.constraint(equalTo: segmentControl.rightAnchor),
             $0.rightAnchor.constraint(equalTo: rightAnchor),
             $0.centerYAnchor.constraint(equalTo: segmentControl.centerYAnchor)]
        }

        addSubview(containerView)
        containerView.makeConstraints {
            [$0.leftAnchor.constraint(equalTo: segmentControl.leftAnchor),
             $0.rightAnchor.constraint(equalTo: rightAnchor),
             $0.bottomAnchor.constraint(equalTo: bottomAnchor),
             $0.topAnchor.constraint(equalTo: segmentControl.bottomAnchor, constant: 10)]
        }
    ...
    func setup(with viewModel: ConnectionDetailViewModel) {
        ...
        viewModel.$entry.weakAssign(to: \.stringValue, on: generalView.networkTypeLabel).store(in: &cancelable)
